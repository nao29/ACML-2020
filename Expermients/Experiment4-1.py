...
K = 8
N = 1000
TEST_EPOCH = 2#テストデータの繰り返し回数
THETA = 2#学習データの繰り返し回数
...
for m in range(M):
#    print('m= %s' % m)
    df =pd.read_csv("nursery5_random9.csv")
    for test in range(TEST_EPOCH):
        print('test= %s' % test)
        test_data = np.array(df.iloc[:, [random.randint(0,12900) for i in range(TEST)]])
        for theta in range(THETA):
            train_data = np.array(df.iloc[:, [random.randint(0,12900) for i in range(N)]])#エラー出るため10個ずつ余裕をもってとっておく    
